...
Order Quantity: For each product, you can enter a quantity in either "Unit(strip)" or "Unit(box)." The application then automatically calculates the "Box Value," "Strip Value," and the final "Net Bill Amount" based on the entered values and a fixed 12% tax/margin factor.
...
Summary Panel: A summary card provides a quick overview of the "Total Order Value" and the "Total Order Value (in Lakhs)."
